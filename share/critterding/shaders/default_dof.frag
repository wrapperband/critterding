...
// 		float focalRange=1550.0;
// 		Blur = clamp(abs(-position.z - focalDistance) / focalRange, 0.0, 1.0);
...
// 			gl_FragColor = color * texture2D( tex, gl_TexCoord[0].st/*, -2.0*/ );
			...
			if(c.a<0.75)	//Simulate alpha test since blur needs alpha channel, alpha test is deprecated anyway
			{
				discard;
			}
			c *= color;
